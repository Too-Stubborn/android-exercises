...
public class CacheCleanUtils {
    ...
    public static void cleanWebViewCache(Context context) {
        ...
        cleanCustomCache(context.getDir("geolocation", 0).getPath());
        deleteFilesAndDirectory(new File(context.getDir("webview", 0).getPath()));
     ...
     * @param file 要删除的根目录
     */
    private static void deleteFilesAndDirectory(File file) {
        if (file.isFile()) {
            file.delete();
            return;
        }
