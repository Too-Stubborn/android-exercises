package com.xuhj.library.util.security;
...
public class SecurityUtils {
    ...
    public static final String MD5(String src) {
        if (src != null) {
            try {
                ...
                return byte2HexString(md5.digest(src.getBytes()));
    ...
    public static final String MD5(byte[] src) {
        if (src != null) {
            try {
                MessageDigest md5 = MessageDigest.getInstance("MD5");
                return byte2HexString(md5.digest(src));
            } catch (NoSuchAlgorithmException e) {
                e.printStackTrace();
            }
        }
    ...
    public static String encodeByAES(String src) {
        try {
            return AESUtils.encrypt(src, AESUtils.AES_KEY);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
    ...
    public static String decodeByAES(String src) {
        try {
            return AESUtils.decrypt(src, AESUtils.AES_KEY);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
